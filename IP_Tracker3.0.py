...
def save_ip(ip, lat, lon, ip_data):
    with open(ip, "a") as c:
        c.write(ip + "\n")
        c.write( f"=====Location=====\n")
        c.write(f"Latitude: {lat}\n")
        c.write(f"Longitude: {lon}\n")
        c.write(f"Maps: https://www.google.com/maps/@{lat},{lon},8z\n")
        c.write(f"Country: {ip_data['country']}\n")
        c.write(f"\n")
        c.write(f"====General Location ===\n")
        c.write(f" Continent Code  : {ip_data["continent_code"]}\n")
        c.write(f"Continent: {ip_data['continent']}\n")
        c.write(f"Region: {ip_data['region']}\n")
        c.write(f"\n")
        c.write(f"======Time Zone ======\n")
        c.write(f" ID              :{ip_data["timezone"]["id"]}\n")
        c.write(f" ABBR            :{ip_data["timezone"]["abbr"]}\n")
        c.write(f" DST             :{ip_data["timezone"]["is_dst"]}\n")
        c.write(f" Offset          :{ip_data["timezone"]["offset"]}\n")
        c.write(f" UTC             :{ip_data["timezone"]["utc"]}\n")
        c.write(f" Current Time    :{ip_data["timezone"]["current_time"]}\n")
        c.write(f"\n")
        c.write(f"===========Connection============\n")
        c.write(f" ASN             :{ip_data["connection"]["asn"]}\n")
        c.write(f" ORG             :{ip_data["connection"]["org"]}\n")
        c.write(f" ISP             :{ip_data["connection"]["isp"]}\n")
        c.write(f" Domain          :{ip_data["connection"]["domain"]}\n") 
...
# Output box
output_box = scrolledtext.ScrolledText(frame, width=60, height=30, bg="black", fg="orange", insertbackground="orange")
